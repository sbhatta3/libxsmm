...
void libxsmm_generator_gemm_load_C( libxsmm_generated_code*             io_generated_code,
  ...
  if (0 == (LIBXSMM_GEMM_FLAG_BETA_0 & i_xgemm_desc->flags)) { /* Beta=1 */
    ...
    /* pure int8 kernel */
      ...
      for ( l_n = 0; l_n < i_n_blocking; l_n++ ) {
        for ( l_m = 0; l_m < l_m_blocking; l_m++ ) {
          /* load 16 bit values into xmm portion of the register change by D- */
          if ( (i_micro_kernel_config->instruction_set == LIBXSMM_X86_AVX512_VL256) || (i_micro_kernel_config->instruction_set == LIBXSMM_X86_AVX512_VL256_CLX)
                || (i_micro_kernel_config->instruction_set == LIBXSMM_X86_AVX512_VL256_CPX)
             ){
            if ( (i_micro_kernel_config->use_masking_a_c != 0) && ( l_m == (l_m_blocking - 1) ) ) {
        ...
        for (; l_m < l_m_blocking; l_m++ ) {
          ...
          libxsmm_x86_instruction_vec_move( io_generated_code,
              ...
              0, 0, 0, 1 );
                ...
                // ((i_micro_kernel_config->instruction_set <= LIBXSMM_X86_ALLFEAT) && (i_micro_kernel_config->instruction_set >= LIBXSMM_X86_AVX512_CPX) &&
                // (i_micro_kernel_config->instruction_set == LIBXSMM_X86_AVX512_VL256)) CHanges by D-
              ) &&
              ( (LIBXSMM_GEMM_PRECISION_BF16 == LIBXSMM_GETENUM_INP( i_xgemm_desc->datatype ) ) && (LIBXSMM_GEMM_PRECISION_BF16 == LIBXSMM_GETENUM_OUT( i_xgemm_desc->datatype ) ) ) ) {
    /* storing downconverted and rounded C accumulator */
    for ( l_n = 0; l_n < i_n_blocking; l_n++ ) {
...
/*
    if (i_micro_kernel_config->instruction_set <= LIBXSMM_X86_AVX2){
      inst_f32_i32 = LIBXSMM_X86_INSTR_VCVTPS2DQ;
      inst_i32_i8 = LIBXSMM_X86_INSTR_VPMOVSDB;
