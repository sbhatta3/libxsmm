...
int main(int argc, char* argv[])
{
  ...
  for ( i = 0; i < num_layers; ++i ) {
    ...
    /* MNIST Specific where everywhere we use relu act except the last FC  layer */
    if (i < num_layers - 1) {
  ...
  /* Each MPI proc redundantly reads the entire dataset  */
  ...
  n_batches = (n_batches/n_procs) * n_procs;
...
#   pragma omp parallel private(i,j,epoch_id,batch_id)
#endif
  {
    ...
    for (epoch_id = 0; epoch_id < n_epochs; epoch_id++) {
      for (batch_id = 0; batch_id < n_batches/n_procs; batch_id++) {
        /*
         *  Initialize input activations with the correct minibatch per rank. Rank i is responsible for minibatches:
         *     i * (n_batches/n_procs), i * (n_batches/n_procs) + 1, ... , i * (n_batches/n_procs) + (n_batches/n_procs) - 1
        */
        if (tid == 0) {
          libxsmm_act[0]  = libxsmm_dnn_link_tensor( libxsmm_layout, input_acts + rank * (n_batches/n_procs) * MB * C[0] + batch_id * MB * C[0], &status ); CHKERR_LIBXSMM_DNN( status );
          CHKERR_LIBXSMM_DNN( libxsmm_dnn_fullyconnected_bind_tensor( libxsmm_fc_layer[0], libxsmm_act[0], LIBXSMM_DNN_REGULAR_INPUT ) );
          libxsmm_label  = libxsmm_dnn_link_tensor( libxsmm_layout2, train_label + rank * (n_batches/n_procs) * MB + batch_id * MB, &status ); CHKERR_LIBXSMM_DNN( status );
        ...
        /* Print loss ince in a while */
        if ((tid == 0) && (batch_id == 0) && (epoch_id % 10 == 0 || epoch_id == n_epochs - 1) && (rank == 0)) {
          float new_loss;
        ...
        for ( i = num_layers-1; i > 0; --i) {
          ...
          #pragma omp barrier
          /* Communication threads overlap allreduce/optimizer of Layer L whereas compute threads may proceed with computation of FC BP of layer L-1 */
          if (tid >= n_comp_threads) {
  ...
  gflop = 0.0;
  for ( i = num_layers-1; i > 0; --i) {
    gflop += (6.0*(double)global_MB*(double)C[i]*(double)C[i+1]*(double)n_epochs*(double)n_batches) / (1000.0*1000.0*1000.0);
  }
  gflop += (4.0*(double)global_MB*(double)C[0]*(double)C[1]*(double)n_epochs*(double)n_batches) / (1000.0*1000.0*1000.0);
